...
test('the timeout covers reading the body after headers arrive', async (t) => {
  delayedBody(t, 200);
  await assert.rejects(fetchText('https://example.com', { timeoutMs: 20 }),
...
test('caller cancellation still interrupts an in-progress body', async (t) => {
  ...
  t.after(() => clearTimeout(timer));
  await assert.rejects(fetchText('https://example.com', {
...
test('a body completed before the deadline is returned', async (t) => {
  delayedBody(t, 5);
  assert.equal(await fetchText('https://example.com', { timeoutMs: 1000 }), 'complete');
});
